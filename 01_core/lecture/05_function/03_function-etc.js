...
(function () {
  console.log('익명함수 즉시 실행! 함수 정의와 동시에 호출');
})();
...
// 예시 1) 경우에 따라 특정 값을 중가 또는 감소를 시켜줘야됨.
...
// 변화 시킬 밧을 전달 받아서 상황에 따라 변화를 줃로고 적용 시키는 함수
...
//특정 값을 2배 증가시키고 싶음
console.log(apply (5, (a) => {return a * 2} ));
// 3배 증가
console.log(apply (5, function(a) {return a * 3} ));


// 예시 2) 로그인 처리시 아이디/비번/이메일/토큰 중 두개를 체크해야된다고 가정
// 각 요소를 체크하는 함수 정의
...
// 로그인 처리용 함수 정의 (매번 경우에 따라서 체크해야 될 두개의 함수 콜백함수로 전달 받기)
const login = (chekc1, check2 ) => {
