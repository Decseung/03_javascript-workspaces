...
str = `백틱으로 감싼 문자열 내의 ""과 ''는 무자열 데이터로 취급`
...
// 해결 1. 개행(줄바꿈)을 표현하는 이스게이프 시퀸스 사용
...
console.log('나의 이름은 ' + lastName + firstName ) // 불편, 여러 문자열 데이터를 +를 사용해 연결

// 해결. 템플릿 리터럴 + 표현식삽입구문(${})

console.log(`나의 이름은 ${lastName + firstName}`)
/*
  * 논리 타입 
  1. boolean
